...
class inria(imdb):
    ...
    def _load_inria_annotation(self, index):
        ...
        boxes = np.zeros((num_objs, 4), dtype=np.uint16)   # (num_objs, 4)
        gt_classes = np.zeros((num_objs), dtype=np.int32)   # (num_objs,)
        ...
        for ix, obj in enumerate(objs):
            # make pixel indecice 0-based
            coor = re.findall('\d+', obj)
            x1 = float(coor[0])
            y1 = float(coor[1])
            x2 = float(coor[2])
            y2 = float(coor[3])
            cls = self._class_to_ind['person']  # 1
            boxes[ix, :] = [x1, y1, x2, y2]
            gt_classes[ix] = cls
            overlaps[ix, cls] = 1.0
            seg_areas[ix] = (x2 - x1 + 1) * (y2 - y1 + 1)

        # returns indices (i,j) of non-zeros elements (1), and the elements themselves
